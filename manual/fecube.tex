...
\eng{We will need following components: TODO

For an overview of componts, see the cheat sheet distributed seperately}

\ned{We zullen volgende componenten gebruiken: TODO

Voor een overzicht van de componenten, zie het overzicht die je apart gekregen hebt}
...
\caption{Using one pin and the ground, we can make a LED burn. Left with breadboard, right as a schematic - Met een pin en de grond kunnen we een LED doen branden. Links met breadboard, rechts als schema}
...
\caption{Blow up view of a LED - Een LED in het groot}
...
\caption{A PN junction as used in a LED - Een PN overgang zoals gebruikt in een LED (\textcopyright Wikimedia Commons S-kei)}
...
\caption{Different NPN and PNP transistors. There is no standard for the pinouts, so use a datasheet, or try it with a LED like we do. If it does not work, use other legs as in this Picture! - Verschillende NPN en PNP transistoren. Er is geen standaard voor de beentjes, refereer naar een datasheet, of probeer het met een LED zoals wij hier doen. Als het niet werkt, gebruik andere beentjes zoals in deze Figuur!}
...
\caption{Using two pins and an NPN gate, we can switch off the LED on both sides - Met twee pinnen en een NPN brug kunnen we de LED afzetten aan beide kanten.}
...
\caption{An NPN transistor when the junction is active - Een NPN transistor als de overgang open staat.}
...
\begin{figure}
  ...
  \includegraphics[width=6cm]{img/02_twopin_oneled_npn_bb} \ \ 
  \includegraphics[width=6cm]{img/02_twopin_oneled_npn_schem}
\caption{The circuit to use PWM to change the brightness. Yes, it is identical to the previous one, all changes are in code - Het circuit om PWM te gebruiken om de helderheid te wijzigen. Ja, het is identiek aan het eerdere circuit, alle wijzigingen zijn in software.}
...
\caption{Using two pins and an NPN gate, we can switch off the LED on both sides. Now we use pin 2 to allow a sink at the pin. Right: Removing the NPN makes for a simpler circuit - Met twee pinnen en een NPN brug kunnen we de LED afzetten aan beide kanten. We gebruiken nu pin 2 zodat we de pin als grond kunnen gebruiken. Rechts: De NPN verwijderen zorgt voor een eenvoudiger circuit.}
...
\caption{Left a blow up of an RGB Led with common anode, Right a schematic version used in circuits - Links een vergroting van een RGB led met gedeelde anode, Rechts een schema zoals je in circuits kan vinden.}
\label{f:RGB_LED}
\end{figure}
...
\caption{Circuit for a common anode RGB Led - Circuit voor een gedeelde anode RGB Led.}
\label{f:lesson2_RGB}
\end{figure}
...
\begin{itemize}
 ...
 \item ....
\end{itemize}
So, how to program this? We will do like in the movies: a specific effect consists of different actions. An action has a certain duration: $t_a$. The action is divided in frames $F$, which is a single image, like a picture taken at a specific instance. So, an action is a function, which for a specific time needs to tell us what frame to show: $F=\mbox{action(t)}$
...
\begin{itemize}
 ...
 \item ....
\end{itemize}
Dus, hoe zullen we dat programmeren? We zullen doen zoals in de filmen: een specifiek effect bestaat uit verschillende acties. Een actie heeft een zekere duur: $t_a$. De actie verdelen we in frames $F$, welke een enkel beeld zijn, zoals een foto genomen op een specifiek moment. Een actie is dus een functie, welke voor een specifieke tijd ons dient te vertellen welk frame te tonen: $F=\mbox{actie(t)}$
...
A subframe consists in showing this red, green or blue. Let's take red. If we show red, we will have a number from 0 to 63 to show. Let's call the number $R$, for example $R=35$. We said a subframe should be around 333$\mu$s. Let's take 64*5 = 320 $\mu$s.
}

\ned{TODO}
\begin{code}\label{c:l2_c}
 \ \newline
%\inputardfull{\string"sketches/Fe_cube_03_RGB_npn_random_color_v4/Fe_cube_03_RGB_npn_random_color_v4.ino\string"}
\end{code}


\subsection{\engt{Smooth transitions} \nedt{Gladde overhangeng}}

\eng{We now extend the previous code, and add a smooth transition from the first color to the next color.}

\ned{TODO}

\begin{code}\label{c:l2_d}
 \ \newline
%\inputardfull{\string"sketches/Fe_cube_04_RGB_npn_smoothrandom_colorv2/Fe_cube_04_RGB_npn_smoothrandom_colorv2.ino\string"}
\end{code}


\section{\engt{Lesson 3: LED Cube Construction} \nedt{Les 3: LED Cube constructie}}

\subsection{Layout - Opstelling}



\section{\engt{Lesson 4: LED Cube Testing} \nedt{Les 4: LED Cube Uittesten}}
