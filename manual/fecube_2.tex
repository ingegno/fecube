...
\ned{In de vorige oefening hebben we geleerd dat een drukknop de stroom kan onderbreken. Dit kan nuttig zijn in sommige omstandigheden, maar niet voor onze Fe Kubus. In de plaats willen we een knop waardoor we input kunnen krijgen van de gebruiker. In andere woorden, we will weten als de gebruiker de knop drukt of niet, en we willen daarop reageren via Arduino code.
...
Een grote weerstand zal zorgen dat de weerstand in de draden van de drukknop naar de 5V pin verwaarloosbaar is ten opzichte van de grote weerstand, en dus zal pin 2 op 5V staan. Terzelfdertijd zal een grote weerstand ervoor zorgen dat er maal heel weinig stroom zal vloeien over de drukknop, en ons circuit zal dus heel efficient zijn. Neem daarom als weerstand eentje van 10000$\Omega$, welke we ook schrijven als 10K$\Omega$. We noemen zo'n weerstand op deze manier gebruikt een \textit{pull-down} of \textit{pull-up} weerstand.
...
\begin{enumerate}
 ...
 \item \textbf{double press}: a short press within 0.7 seconds after a previous short press
 \item \textbf{extreme long press}: a press longer than 5 seconds. No need to wait for the release to set a press type.
\end{enumerate}
...
\begin{itemize}
 ...
 \item Een \textbf{event} of \textbf{gebeurtenis} is iets dat gebeurt met de knop. Er zijn maar twee mogelijke events : \textit{buttonpress} and \textit{buttonrelease}, in het Nederlands: Knop indrukken en Knop loslaten. Zoals we in de vorige oefening gezien hebben kunnen we deze herkennen door een verandering in de status. De eerste is LOW naar HIGH en de tweede HIGH naar LOW.
 \item Vervolgens hebben we \textbf{event handling}. Event handling is het omzetten van events naar \textit{button press type}. Om event handling te doen moeten we tijd weten sinds een vorig event alsook het type van de vorige butten press. Op deze manier kunnen we onderscheiden:
\begin{enumerate}
 \item \textbf{short press} of \textbf{korte druk}: een druk en loslaten binnen de 2 sekonden
 \item \textbf{long press} of \textbf{lange druk}: een druk en loslaten die langer dan 2 sekonden duurt
 \item \textbf{double press} of \textbf{dubbele druk}: a korte druk binnen de 0.7 sekonden na een vorige korte druk
 \item \textbf{extreme long press} of \textbf{extreem lange druk}: een druk die langer dan 5 sekonden duurt. Het is dan niet nodig te wachten op een loslaten om het druk type vast te leggen.
\end{enumerate}
